...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.zo1jc.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
MongoClient.connect(uri, { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
    const customerCollection = client.db("mySecondDatabase").collection("creative-customer");
    const reviewCollection = client.db("mySecondDatabase").collection("customer-review");
    const adminCollection = client.db("mySecondDatabase").collection("admin");
    const serviceCollection = client.db("mySecondDatabase").collection("services");
